...
/**
 * first implement kd-tree node, we use left, right pointer to is children and in node, will save it
 * is vertical or horizontal, and save it is back rectangle the root back rectangle is (0,0,1,1)
 * <p>
 ...
 * <p>
 * after build the kd-tree we can implement the nearest and range
 */
public class KdTree {
    ...
    private static class Node {
        ...
        public int compare(RectHV queryRect) {
            return isVertical ? Double.compare(queryRect.xmax(), p.x()) :
                   Double.compare(queryRect.ymax(), p.y());
        }
    ...
    private RectHV buildRect(Node prev, Point2D p) {
        ...
        if (prev.isVertical) {
            if (isLeft)
                return new RectHV(prev.rect.xmin(), prev.rect.ymin(), prev.p.x(), prev.rect.ymax());
            else
                return new RectHV(prev.p.x(), prev.rect.ymin(), prev.rect.xmax(), prev.rect.ymax());
        }
        if (isLeft)
            return new RectHV(prev.rect.xmin(), prev.rect.ymin(), prev.rect.xmax(), prev.p.y());
    ...
    private RectHV buildLine(Node cur) {
        if (cur.isVertical)
            return new RectHV(cur.p.x(), cur.rect.ymin(), cur.p.x(), cur.rect.ymax());
    ...
    /**
     * 0. add node.point in result if it is in query rectangle 1. the diving line in kd-tree is
     * intersect the query rectangle 2. if yes, search both node 3. if no, go the child which is
     * closer to query rectangle
     *
     * @param cur
     * @param rect
     * @param inRange
    ...
    /**
     * 0. if current point is nearest than the current nearest ,update it. 1. compare the node
     * should be left subtree or right subtree 2. search that tree 3. check other tree when the
     * target node distance to the other node's background rectangle is smaller than the current min
     * distance.
     *
     * @param cur
     * @param tar
    ...
    private void nearest(Node cur, Point2D tar, Node result) {
        ...
        if (otherNode != null && Double.compare(otherNode.rect.distanceSquaredTo(tar),
                                                result.p.distanceSquaredTo(tar))
                <= 0) { // critical is smaller
